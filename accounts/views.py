...
#restrick customer from accessing vendor page
...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            messages.success(request, "Account Created Successfully. Please check your email to verify. Thanks")
...
def registerVendor(request):
    if request.user.is_authenticated:
        ...
        if u_form.is_valid() and v_form.is_valid():
            ...
            messages.success(request, "Account Created Successfully. Please check your email to verify.   Vendor accounts will be approved by the admin.  Thanks for your patience.")
...
def vendorDashboard(request):
    return render(request, 'accounts/dashboard-v.html')
...
def reset_password_validate(request, uidb64, token):
    #decode the token to validate user
    try:
        uid = urlsafe_base64_decode(uidb64).decode()
        user = User._default_manager.get(pk=uid)
    except(TypeError, ValueError, OverflowError, User.DoesNotExist):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, "This link may have expired")
        return redirect('accounts:myaccount')
